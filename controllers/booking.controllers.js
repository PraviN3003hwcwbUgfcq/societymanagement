...
const getBookings = asyncHandler(async (req, res) => {
  const allBookings = await Booking.find({ societyId: req.user?.societyId })
    .sort({ date: -1 })
    .select("-__v -updatedAt -societyId")
    .populate("bookingOwner", "houseNo block -_id")
    .lean();
...
const getPastBookingsByUserId = asyncHandler(async (req, res) => {
  ...
  const allBookings = await Booking.find({
    ...
    .select("-__v -updatedAt -bookingOwner -societyId")
    .lean();
...
const payBooking = asyncHandler(async (req, res) => {
  ...
  const venues = await Venue.find({
    venue: bookings.bookingType,
    societyId: req.user?.societyId,
  });

  if (!venues || venues.length === 0) {
    throw new ApiError(404, "Venue not found for this booking");
  }
...
const saveBookingOrder = asyncHandler(async (req, res) => {
  ...
  if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !bookingId || !amount) {
    throw new ApiError(400, "Missing required payment/order fields");
  }
...
const getBookingOrdersForUser = asyncHandler(async (req, res) => {
  ...
  const orders = await BookingOrder.find({ userId })
    .select("bookingId paymentIntentId status paidOn amount receiptUrl")
    .lean();
