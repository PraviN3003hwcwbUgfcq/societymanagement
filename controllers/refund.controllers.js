...
const requestRefund = asyncHandler(async (req, res) => {
  ...
  const OrderModel = orderType === "EventOrder" ? EventOrder : BookingOrder;
  const order = await OrderModel.findById(id);
  ...
  const razorpayPaymentId = order.paymentIntentId;

  if (!razorpayPaymentId) {
    throw new ApiError(400, "Payment ID not found for refund");
  }
  ...
  if (hoursElapsed <= 24) {
    try {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, {}, "Refund processed automatically."));
    } catch (error) {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, refundReq, "Refund request submitted for admin review."));
});
...
const getPendingRefunds = asyncHandler(async (req, res) => {
  ...
  const societyRequests = requests.filter(
    (r) =>
      r.user &&
      r.user.societyId === req.user.societyId &&
      r.user._id.toString() !== req.user._id.toString()
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, societyRequests, "Pending refund requests fetched"));
});
...
const approveRefund = asyncHandler(async (req, res) => {
  ...
  if (refundReq.status !== "Pending") {
    throw new ApiError(400, "Request is not pending. Current status: " + refundReq.status);
  }

  if (refundReq.user.societyId !== req.user.societyId) {
    throw new ApiError(403, "Not authorized for this society");
  }

  if (refundReq.user._id.toString() === req.user._id.toString()) {
    throw new ApiError(403, "You cannot approve your own refund request");
  }

  try {
    await razorpay.payments.refund(refundReq.paymentIntentId);

    refundReq.status = "Approved";
    await refundReq.save();

    const OrderModel = refundReq.orderType === "EventOrder" ? EventOrder : BookingOrder;
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, refundReq, "Refund approved and processed successfully."));
  } catch (error) {
    throw new ApiError(500, `Razorpay refund failed: ${error.message}`);
  }
});
...
const rejectRefund = asyncHandler(async (req, res) => {
  ...
  if (refundReq.user._id.toString() === req.user._id.toString()) {
    throw new ApiError(403, "You cannot reject your own refund request");
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, refundReq, "Refund request rejected."));
});
